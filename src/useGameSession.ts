...
export async function createGameSession() {
  ...
  return {
    ...
    observeIncident: () => runCommand({ type: 'observe_incident' }),
    reconIncident: () => runCommand({ type: 'recon_incident' }),
    scanIncident: () => runCommand({ type: 'scan_incident' }),
    contactIncident: () => runCommand({ type: 'contact_incident' }),
    requestPoliceSupport: () => runCommand({ type: 'request_police_support' }),
    requestMonolithSupport: () => runCommand({ type: 'request_monolith_support' }),
    resolvePoliceContainer: (decision: ContainerDecision) => runCommand({ type: 'resolve_police_container', decision }),
