...
<template>
  <label :class="{ pending }">
    ...
    <select v-memo="[draftItemId, equippedItemId, visibleInventoryKey]" :value="draftItemId" :disabled="slot === 'suit'" @pointerdown="beginInteraction" @focus="beginInteraction" @input="handleSelection" @change="handleSelection" @blur="finishInteraction">
      ...
      <option v-for="item in items" :key="item.id" :value="item.id" :disabled="!canSelectEquipmentItem(state, cat.id, slot, item.id)">{{ tr('item.stock', { item: item.name, count: visibleInventory[item.id] }) }}</option>
    </select>
  </label>
</template>
