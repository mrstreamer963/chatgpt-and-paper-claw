  ...
  <div v-if="state.incident && state.incident.stage !== 'support_en_route'" class="incident-overlay">
    <section class="incident-card" role="dialog" aria-modal="true" aria-labelledby="incident-title">
      ...
      <template v-if="state.incident.stage === 'decision'">
        ...
        <div class="incident-facts"><span>{{ tr('ОТРЯД') }} <b>{{ tr('cats.count', { count: primarySquad?.members.length ?? 0 }) }}</b></span><span>{{ tr('ПРОГРЕСС') }} <b>{{ Math.round(GAME_RULES.raidTriggerProgress / GAME_RULES.cleanupDuration * 100) }}%</b></span><span>{{ tr('НАГРАДА ПОД УГРОЗОЙ') }} <b>{{ tr('scrap.count', { count: GAME_RULES.cleanupRewardScrap }) }}</b></span></div>
