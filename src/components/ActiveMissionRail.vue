...
import { GAME_RULES, getCleanupSecondsRemaining, getMissionSquadRosters, isActiveAssignedMission, type Mission, type State } from '@nine-lives/game-core'
...
<template>
  <aside class="active-mission-rail">
    ...
    <button v-for="mission in activeMissions" :key="mission.id" type="button" class="mission-rail-card" :class="{ assigned: mission.status === 'assigned', selected: selectedMissionId === mission.id }" @click="emit('select', mission)">
      ...
      <span v-if="squadRosters(mission).length" class="mission-squads">
        <span v-for="roster in squadRosters(mission)" :key="roster.squad.id" class="mission-squad">
