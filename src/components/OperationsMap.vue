...
const separatedSquadPositions = computed(() => layoutSquadMarkers(props.state.squads.filter(squad => squad.phase !== 'base').map(squad => ({ id: squad.id, ...squadPosition(squad) })), mapSize.value))
function separatedSquadPosition(squad: Squad) { return separatedSquadPositions.value.get(squad.id) ?? squadPosition(squad) }
function squadStyle(squad: Squad) { const position = separatedSquadPosition(squad); return { left: `${position.x}%`, top: `${position.y}%`, '--squad-color': squadColor(squad) } }
...
<template>
  <section class="map-view">
    <div ref="mapGrid" class="map-grid" :class="{ 'incident-active': state.incident, 'command-active': selectedCount() || selectedTarget, 'merge-active': mergeSourceSquadId }" @click="selectMapPoint" @pointerdown="beginSelection" @pointermove="updateSelection" @pointerup="finishSelection" @pointercancel="selectionBox = undefined">
      ...
      <button v-for="squad in state.squads.filter(candidate => candidate.phase !== 'base')" :key="squad.id" type="button" class="squad-marker" :class="[squad.phase, squad.id, squadIndex(squad) % 2 ? 'callout-right' : 'callout-left', { selected: selectedSquadIds.includes(squad.id), available: squadIsAvailable(squad) }]" :style="squadStyle(squad)" @click.stop="selectSquad(squad, $event)"><div class="map-squad-tokens"><svg v-for="member in squad.members" :key="member" class="cat-silhouette" viewBox="0 0 64 64" aria-hidden="true"><use :href="`${catTokensUrl}#token-${member}`" /></svg></div><span class="squad-callout"><b>{{ squadDisplayName(locale, squad) }}</b></span></button>
